...
static loom_err_t loom_bitmap_min_stride(const loom_bitmap_t *bitmap,
                                         size_t *min_stride) {
...
static char *loom_strdup_internal(const loom_t *loom, const char *text) {
  size_t len = strlen(text);
  if (len == SIZE_MAX) {
    return NULL;
  }

  char *copy = loom_platform_malloc(loom, len + 1, LOOM_ALLOC_INTERNAL);
  if (copy == NULL) {
    return NULL;
  }
...
static loom_err_t loom_require_frame(const loom_t *loom) {
  if (loom == NULL) {
    return LOOM_ERR_INVALID_ARG;
  }
  if (!loom->in_frame) {
...
void loom_release_frame_texts(loom_t *loom) {
  ...
  for (size_t i = 0; i < loom->command_count; ++i) {
    ...
    if (command->type == LOOM_CMD_TEXT && command->data.text.text != NULL) {
      loom_platform_free(loom, command->data.text.text);
...
loom_err_t loom_invalidate_rect(loom_t *loom, loom_rect_t rect) {
  loom_err_t ret = loom_require_frame(loom);
  if (ret != LOOM_OK) {
    return ret;
  }
...
loom_err_t loom_push_clip(loom_t *loom, loom_rect_t rect) {
  loom_err_t ret = loom_require_frame(loom);
  if (ret != LOOM_OK) {
    return ret;
  }
...
loom_err_t loom_pop_clip(loom_t *loom) {
  loom_err_t ret = loom_require_frame(loom);
  if (ret != LOOM_OK) {
    return ret;
  }
...
loom_err_t loom_draw_bitmap(loom_t *loom, loom_rect_t dst,
  ...
  size_t min_stride = 0;
  if (loom_bitmap_min_stride(bitmap, &min_stride) != LOOM_OK ||
...
loom_err_t loom_draw_text(loom_t *loom, const loom_font_t *font,
  ...
  char *text_copy = loom_strdup_internal(loom, text);
  if (text_copy == NULL) {
    return LOOM_ERR_NO_MEM;
  }
  ...
  if (ret != LOOM_OK) {
    loom_platform_free(loom, text_copy);
  }
  return ret;
}
